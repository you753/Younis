...
export default function FinancialReports() {
  ...
  const totalExpenses = filteredPurchases.reduce((sum, purchase) => sum + parseFloat(purchase.total), 0);
  const totalSalaryExpenses = filteredSalaries.reduce((sum, salary) => sum + parseFloat(salary.netSalary), 0);
  const totalCosts = totalExpenses + totalSalaryExpenses;
  ...
  const monthlyData = Array.from({ length: 12 }, (_, i) => {
    ...
    return {
      month: `${month}`,
      monthName: new Date(2024, i, 1).toLocaleDateString('ar-SA', { month: 'long' }),
      sales: monthSales,
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 bg-orange-50 rounded-lg">
                ...
                <span className="font-bold text-orange-600">{totalExpenses.toFixed(2)} ر.س</span>
              </div>
                ...
                <>
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span>المشتريات</span>
                      <span>{((totalExpenses / totalCosts) * 100).toFixed(1)}%</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div 
                        className="bg-orange-600 h-2 rounded-full" 
                        style={{ width: `${(totalExpenses / totalCosts) * 100}%` }}
                      ></div>
                    </div>
                  </div>
                  <div className="space-y-2">
