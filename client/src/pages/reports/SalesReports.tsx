...
export default function SalesReports() {
  ...
  const exportToPDF = () => {
    try {
      ...
      doc.text('moassasat almohasseb alazam', 20, 20);
      doc.text('almamlakat alarabiat alsaudia', 20, 28);
      doc.text('mantiqat jidda', 20, 36);
      doc.text('raqam darribat alqiema almudafa: 311852766100003', 20, 44);
      doc.text('hatef: 0552490756', 20, 52);
      
      // الجانب الأيمن - معلومات الشركة
      doc.text('مؤسسة المحاسب الأعظم التجارية', 120, 20);
      doc.text('المملكة العربية السعودية', 120, 28);
      doc.text('منطقة جدة', 120, 36);
      doc.text('رقم ضريبة القيمة المضافة: 311852766100003', 120, 44);
      doc.text('هاتف: 0552490756', 120, 52);
      
      // العنوان الرئيسي
      doc.setFontSize(16);
      doc.setTextColor(0, 0, 0);
      doc.text('تقرير المبيعات', 105, 70, { align: 'center' });
      
      // تاريخ التقرير
      doc.setFontSize(10);
      const currentDate = new Date().toLocaleDateString();
      doc.text(`فترة التقرير: ${dateFrom || 'الكل'} - ${dateTo || 'الكل'}`, 105, 85, { align: 'center' });
      ...
      doc.text('التاريخ', 55, yPosition);
      doc.text('اسم العميل', 85, yPosition);
      doc.text('المبلغ ريال', 135, yPosition);
      doc.text('الحالة', 165, yPosition);
      ...
      filteredSales.forEach((sale, index) => {
        ...
        // اسم العميل
        const clientName = client?.name || 'عميل نقدي';
        doc.text(clientName, 85, yPosition);
        
        doc.text(`${parseFloat(sale.total).toFixed(2)}`, 135, yPosition);
        doc.text('مكتمل', 165, yPosition);
      ...
      doc.setFontSize(10);
      doc.setTextColor(0, 0, 0);
      doc.text('إجمالي المبيعات:', 85, yPosition);
      doc.text(`${totalSales.toFixed(2)} ريال`, 135, yPosition);
      
      // تذييل التقرير
      const pageHeight = doc.internal.pageSize.height;
      doc.setFontSize(8);
      doc.setTextColor(100, 100, 100);
      doc.text('تم إنشاؤه بواسطة نظام المحاسب الأعظم', 105, pageHeight - 20, { align: 'center' });
