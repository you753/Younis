...
const salaryFormSchema = z.object({
  ...
  amount: z.string().min(1, 'مبلغ الراتب مطلوب'),
...
export default function SalaryFormComponent({ 
  ...
  const form = useForm<SalaryForm>({
    ...
    defaultValues: {
      ...
      bonus: editingSalary?.bonus || '0',
      deductions: editingSalary?.deductions || '0',
      netAmount: editingSalary?.netAmount || '',
      status: editingSalary?.status || 'pending',
      paymentDate: editingSalary?.paymentDate ? new Date(editingSalary.paymentDate).toISOString().split('T')[0] : '',
