...
async function initializeDatabaseWithSampleData() {
  ...
  // Accounting System Implementation - Account Categories
  ...
  async getAllInvoiceTemplates(): Promise<InvoiceTemplate[]> {
    return db.select().from(invoiceTemplates).orderBy(desc(invoiceTemplates.id));
  }

  async getInvoiceTemplatesByType(type: string): Promise<InvoiceTemplate[]> {
    return db.select().from(invoiceTemplates).where(eq(invoiceTemplates.type, type));
  }

  async getDefaultInvoiceTemplate(type: string): Promise<InvoiceTemplate | undefined> {
    const [template] = await db.select().from(invoiceTemplates)
      .where(sql`${invoiceTemplates.type} = ${type} AND ${invoiceTemplates.isDefault} = true`)
      .limit(1);
    return template || undefined;
  }
  ...
  async setDefaultInvoiceTemplate(id: number, type: string): Promise<boolean> {
    await db.update(invoiceTemplates).set({ isDefault: false }).where(eq(invoiceTemplates.type, type));
    const result = await db.update(invoiceTemplates).set({ isDefault: true }).where(eq(invoiceTemplates.id, id));
    return result.rowCount ? result.rowCount > 0 : false;
  }
  ...
  async getAllReportTemplates(): Promise<ReportTemplate[]> {
    return db.select().from(reportTemplates).orderBy(desc(reportTemplates.id));
  }

  async getReportTemplatesByType(type: string): Promise<ReportTemplate[]> {
    return db.select().from(reportTemplates).where(eq(reportTemplates.type, type));
  }

  async getDefaultReportTemplate(type: string): Promise<ReportTemplate | undefined> {
    const [template] = await db.select().from(reportTemplates)
      .where(sql`${reportTemplates.type} = ${type} AND ${reportTemplates.isDefault} = true`)
      .limit(1);
    return template || undefined;
  }
  ...
  async setDefaultReportTemplate(id: number, type: string): Promise<boolean> {
    await db.update(reportTemplates).set({ isDefault: false }).where(eq(reportTemplates.type, type));
    const result = await db.update(reportTemplates).set({ isDefault: true }).where(eq(reportTemplates.id, id));
    return result.rowCount ? result.rowCount > 0 : false;
  }
}
